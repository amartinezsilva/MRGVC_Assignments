...
def compute_and_visualize_poses(E, x1, x2, X_w):
    ...
    for idx, (R, t) in enumerate(poses):
        ...
        plot_3D(X_computed, X_w, T_w_c1, np.linalg.inv(T_c2_c1), idx+1)
...
if __name__ == '__main__':
    ...
    X_w = np.loadtxt('X_w.txt')  #If no GT, use X_c1_w
    T_w_c1 = np.loadtxt('T_w_c1.txt')   #If no GT, should be np.eye(4)
    # #Bring ground truth points to camera 1 reference (fixed)
    X_c1_w = np.dot(np.linalg.inv(T_w_c1),X_w)
    T_w_c1 = np.eye(4)
    X_w = X_c1_w
    
    #T_w_c2 = np.loadtxt('T_w_c2.txt')   #If no GT, you don't know this!
    ...
    R_c2_c1_chosen, t_c2_c1_chosen, min_error, X_computed = compute_and_visualize_poses(E, x1Data, x2Data, X_w)
