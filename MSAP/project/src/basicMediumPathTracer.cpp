            ...
            if(validIntersection && its.mesh->hasMedium() &&
               its.mesh->getMedium()->sampleDistance(Ray3f(pathRay, 0.0, its.t), mRec, sampler->next2D())){
                ...
                if(!validIntersection) {
                    const Emitter* envLight = scene->getEnvironmentalEmitter();
                ...
                if (its.mesh->isEmitter() ) {
                    ...
                    Color3f Le_emitter = its.mesh->getEmitter()->sample(emR, sampler->next2D(), 0.);
                    Li += tp * Le_emitter; //aEM->sampleL(-pathRay.d, its.shFrame.n, its);
                }
                ...
                BSDFQueryRecord bRec(its.toLocal(-pathRay.d),
								Vector3f(0.0f), its.uv, ESolidAngle);
